...
function __set_ha_defaults {
    local DEFAULT_HA_HEARTBEAT_AUTH_MD5SUM=$(echo -n ${KUBERNETES_CONTROLPLANE_IP}:${KUBERNETES_CONTROLPLANE_BIND_PORT} | md5sum | awk '{print $1}')
...
function __set_kubernetes_defaults {
    ...
    local DEFAULT_KUBERNETES_APISERVER_LOCAL_BIND_PORT="6443"
    if [[ $ENABLE_HA ]]; then
        DEFAULT_KUBERNETES_APISERVER_LOCAL_BIND_PORT="16443"
    fi
    local DEFAULT_KUBERNETES_OIDC_ISSUER_URL=https://accounts.google.com
    local SNAPROUTE_KUBERNETES_OIDC_CLIENT_ID=116110509010-2l4hmt7695nnabm82dbtrgo6sf1p9hf9.apps.googleusercontent.com
    ...
    if [[ -z "${KUBERNETES_APISERVER_LOCAL_BIND_PORT}" ]]; then
        KUBERNETES_APISERVER_LOCAL_BIND_PORT=${DEFAULT_KUBERNETES_CONTROLPLANE_BIND_PORT}
    fi
